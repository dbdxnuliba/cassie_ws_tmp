...
CassiePlugin::CassiePlugin() :
    ...
    kMaxTorque_{4.5, 4.5, 12.2, 12.2, 9.0, 4.5, 4.5, 12.2, 12.2, 9.0},
...
void CassiePlugin::Reset() {
    ...
    gazebo::physics::JointPtr yaw_joint = model->GetJoint("BaseRotZ");
...
//    std::cout << "controller calculated u & actual u" << u << ", " << tau << std::endl;
...
void CassiePlugin::Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf)
{
    ...
    // removed left-knee-shin-joint & right-knee-shin-joint
    joint_ = {
...
void CassiePlugin::onUpdate()
{
    ...
    const double HOLD_TIME  = 4.4;
    const double DETACH_TIME = 5.0;
//
//    const double LOWER_TIME = 50.0;
//    const double HOLD_TIME  = 50.4;
//    const double DETACH_TIME = 50.0;
    const double DAMPER_TIME = 5.5;
...
void CassiePlugin::updateCassieOut()
{
    ...
    for (size_t i = 0; i < 6; ++i) {
        if (i==0 || i==3){
            jointOut_[i]->position = 0;
            jointOut_[i]->velocity = 0;
        }else{
...
void CassiePlugin::lowerPelvis()
{
    ...
    joint->SetLowerLimit(0, joint->LowerLimit()-0.00005); //0.00005
//    joint->SetLowerLimit(0,-1000);
//    joint->SetPosition(0,joint->Position(0)-0.00005);
}
