...
void OUTPUT_SLIP::Splines::updateNominalSplines(double desiredStepLength) {
    ...
    this->z_scaling = 2.0*(poly1*(stepLengthDesired) + poly2);
    ...
    VectorXd zcom_cassie = (this->zcom_human.array() - this->zcom_human.mean())*this->z_scaling + this->h_cassie;
...
void OUTPUT_SLIP::Splines::updateDownstepSplines(double desiredStepLength){
    ...
    BasicSpline zcom_cassie_spline_1(time_norm_local_1,zcom_cassie_1,dzcom_cassie_1);
    ...
    BasicSpline dzcom_cassie_spline_1(time_norm_local_1,dzcom_cassie_1,ddzcom_cassie_1);
    ...
    BasicSpline ddzcom_cassie_spline_1(time_norm_local_1,ddzcom_cassie_1,ddzcom_cassie_1);
    ...
    BasicSpline xcom_cassie_spline(time_norm_local_1,xcom_cassie,dxcom_cassie);
    ...
    BasicSpline dxcom_cassie_spline(time_norm_local_1,dxcom_cassie,ddxcom_cassie);
    ...
    BasicSpline ddxcom_cassie_spline(time_norm_local_1,ddxcom_cassie,ddxcom_cassie);
    ...
    BasicSpline GRF_SSP_spline(time_norm_local_SSP_1,grf_cassie_SSP_1,grf_cassie_SSP_1);
...
void OUTPUT_SLIP::update(VectorXd &radio) {
    ...
    if (this->config.control_mode == 1){ //walking
        ...
        min = this->splines.debugSplines.timeMin[this->splines.debugSplines.idx];
        max = this->splines.debugSplines.timeMax[this->splines.debugSplines.idx];
    ...
    // in air
        ...
        this->computeDesired_inair(this->cache.yd, this->cache.dyd, this->cache.d2yd, radio);
    }
}
...
void OUTPUT_SLIP::computeDesired_inair(VectorXd &yd, VectorXd &dyd, VectorXd &d2yd, VectorXd &radio) {
    yd.setZero();
    dyd.setZero();
    d2yd.setZero();

    double xRatio = 0.2, yRatio = 0.2, zRatio = 0.2;
    yd(0) =  0.0   + xRatio*radio(RadioButtonMap::LV);
    yd(1) =  0.045 + yRatio*radio(RadioButtonMap::LH);
    yd(2) = -0.95  + zRatio*radio(RadioButtonMap::LS);

    yd(5) =  0.0   + xRatio*radio(RadioButtonMap::RV);
    yd(6) = -0.045 + yRatio*radio(RadioButtonMap::RH);
    yd(7) = -0.95  + zRatio*radio(RadioButtonMap::RS);
