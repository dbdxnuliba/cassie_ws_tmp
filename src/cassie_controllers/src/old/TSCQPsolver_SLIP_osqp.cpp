...
VectorXd TSCQPsolver_SLIP::getTorqueQP(OUTPUT_SLIP &output) {
    ...
    // OSQP //
    //////////
    // Add direct decision variable constraints in lb<=Ax<=ub
    Eigen::MatrixXd AconstrLarge; AconstrLarge.resize(this->config.nC+this->config.nV,this->config.nV); AconstrLarge.setZero();
    Eigen::VectorXd lbALarge; lbALarge.resize(this->config.nC+this->config.nV); lbALarge.setZero();
    Eigen::VectorXd ubALarge; ubALarge.resize(this->config.nC+this->config.nV); ubALarge.setZero();
    AconstrLarge.block(0,0,this->config.nC,this->config.nV) = this->cache.Aconstr;
    AconstrLarge.block(this->config.nC,0,this->config.nV,this->config.nV) = MatrixXd::Identity(this->config.nV,this->config.nV);
    lbALarge << this->cache.lbA, this->config.lb;
    ubALarge << this->cache.ubA, this->config.ub;
    // Convert Matrix to SparseMatrix
    Eigen::SparseMatrix<double> Gsparse = this->cache.G.sparseView();
    Eigen::SparseMatrix<double> Aconstrsparse = AconstrLarge.sparseView();
    // Set the QP
    auto before = std::chrono::high_resolution_clock::now();
    OsqpEigen::Solver solver;
    solver.settings()->setVerbosity(false);
    solver.data()->setNumberOfVariables(this->config.nV);
    solver.data()->setNumberOfConstraints(this->config.nC+this->config.nV);
    solver.data()->setHessianMatrix(Gsparse);
    solver.data()->setGradient(this->cache.g);
    solver.data()->setLinearConstraintsMatrix(Aconstrsparse);
    solver.data()->setLowerBound(lbALarge);
    solver.data()->setUpperBound(ubALarge);
    // Solve the QP
    solver.initSolver();
    solver.solve();
    auto after  = std::chrono::high_resolution_clock::now();
    auto dt = after - before;
    osqp_time += std::chrono::duration_cast<std::chrono::microseconds>(dt);
    std::cout << "SOLUTION OSQP:    " << solver.getSolution() << "\t["<<osqp_time.count()<<" mus]" << std::endl;


    // // '' working '' 
    // // Convert Matrix to SparseMatrix
    // Eigen::SparseMatrix<double> Gsparse = this->cache.G.sparseView();
    // Eigen::SparseMatrix<double> Aconstrsparse = this->cache.Aconstr.sparseView();
    // // Set the QP
    // OsqpEigen::Solver solver;
    // solver.settings()->setVerbosity(false);
    // solver.settings()->setAlpha(1.0);
    // solver.data()->setNumberOfVariables(this->config.nV);
    // solver.data()->setNumberOfConstraints(this->config.nC);
    // solver.data()->setHessianMatrix(Gsparse);
    // solver.data()->setGradient(this->cache.g);
    // solver.data()->setLinearConstraintsMatrix(Aconstrsparse);
    // solver.data()->setLowerBound(this->cache.lbA);
    // solver.data()->setUpperBound(this->cache.ubA);
    // // Solve the QP
    // solver.initSolver();
    // auto before = std::chrono::high_resolution_clock::now();
    // solver.solve();
    ...
    dt = after - before;
    qpoases_time += std::chrono::duration_cast<std::chrono::microseconds>(dt);
    ...
    this->cache.qpsol = solver.getSolution();
...
//        this->memory.u_prev << this->cache.qpsol.segment(this->config.qsize,this->config.usize);
    ...
    std::cout << "SOLUTION QPOASES: " << this->cache.qpsol << "\t["<<qpoases_time.count()<<" mus]" << std::endl;
