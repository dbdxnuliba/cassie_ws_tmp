...
void TSCQPsolver_DEBUG::Cache::init(Config* config) {
...
void TSCQPsolver_DEBUG::Cache::reset() {
    this->Jc.setZero();
    this->dJc.setZero();
    this->Js.setZero();
    this->dJs.setZero();
...
void TSCQPsolver_DEBUG::Memory::init() {
    this->u_prev.resize(10);
    this->reset();
}

void TSCQPsolver_DEBUG::Memory::reset() {
    this->qp_initialized = false;
    this->u_prev.setZero();
}

void TSCQPsolver_DEBUG::Config::init() {
    this->W_outputs.resize(this->outputsize,this->outputsize);
    this->Reg_Matrix.resize(this->nV,this->nV);
}

void TSCQPsolver_DEBUG::Config::reconfigure() {
    this->isSim = false;
...
TSCQPsolver_DEBUG::TSCQPsolver_DEBUG(ros::NodeHandle &nh, cassie_model::Cassie &robot, int control_mode) : nh(&nh), tarsusSolver(robot) {
    this->robot = &robot;
    this->config.control_mode = control_mode;

    if (this->config.control_mode == 1) {
        //walking with line contact
        this->config.outputsize = 9; // 9 (under) or 10 (fully)
...
void TSCQPsolver_DEBUG::reset() {
    this->cache.reset();
    this->memory.reset();
}

bool TSCQPsolver_DEBUG::reconfigure() {
    std::cout << "Polling QP rosparams under: " << this->config.paramChecker.node.getNamespace() << std::endl;
    this->config.reconfigure();
    return true;
}

void TSCQPsolver_DEBUG::update(VectorXd &u, OUTPUT_DEBUG &output) {
    this->cache.u = this->getTorqueQP(output);
    u = this->cache.u;
}

VectorXd TSCQPsolver_DEBUG::getTorqueQP(OUTPUT_DEBUG &output) {
    // Formulate the QP
...
void TSCQPsolver_DEBUG::getDebug(VectorXf &dbg) {
...
/*
VectorXd TSCQPsolver_DEBUG::getTorqueID(OUTPUT_DEBUG &output) {
