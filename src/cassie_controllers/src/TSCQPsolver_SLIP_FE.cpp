...
void TSCQPsolver_SLIP_FE::Cache::init(Config* config) {
...
void TSCQPsolver_SLIP_FE::Cache::reset() {
    this->Jc.setZero();
    this->dJc.setZero();
    this->Js.setZero();
    this->dJs.setZero();
...
void TSCQPsolver_SLIP_FE::Memory::init() {
    this->u_prev.resize(10);
    this->reset();
}

void TSCQPsolver_SLIP_FE::Memory::reset() {
    this->qp_initialized = false;
    this->u_prev.setZero();
}

void TSCQPsolver_SLIP_FE::Config::init() {
...
void TSCQPsolver_SLIP_FE::Config::reconfigure() {
    this->isSim = false;
    ...
    if (this->control_mode == 1) {
        ...
        this->paramChecker.checkAndUpdate("walking_qp/c",this->c);
...
TSCQPsolver_SLIP_FE::TSCQPsolver_SLIP_FE(ros::NodeHandle &nh, cassie_model::Cassie &robot, int control_mode) : nh(&nh), tarsusSolver(robot) {
    this->robot = &robot;
    ...
    this->config.nC = this->config.qsize + this->config.othercons_size + this->config.holsize + this->config.fesize;
...
void TSCQPsolver_SLIP_FE::reset() {
    this->cache.reset();
    this->memory.reset();
}

bool TSCQPsolver_SLIP_FE::reconfigure() {
    std::cout << "Polling QP rosparams under: " << this->config.paramChecker.node.getNamespace() << std::endl;
    this->config.reconfigure();
    return true;
}

void TSCQPsolver_SLIP_FE::update(VectorXd &u, OUTPUT_SLIP &output) {
...
VectorXd TSCQPsolver_SLIP_FE::getTorqueQP(OUTPUT_SLIP &output) {
    // Formulate the QP
    ...
    // force embedding constraint term
    double Fssp = output.splines.nominalSplines.GRF_SSP.evaluate(output.phase.tau,0);
    MatrixXd one; one.resize(1,1); one(0,0) = 1.0;
    this->cache.Aconstr.block(this->config.qsize+this->config.othercons_size+this->config.holsize,this->config.qsize+this->config.usize+2,1,1) = one;

    if (output.memory.phaseCnt < output.config.useHumanOutputsXswStep){
        this->cache.lbA.segment(this->config.qsize+this->config.othercons_size+this->config.holsize,1).setConstant(-INFTY);
        this->cache.ubA.segment(this->config.qsize+this->config.othercons_size+this->config.holsize,1).setConstant(INFTY);
    } else {
        double lbFE = (1.0 - this->config.c)*Fssp;
        double ubFE = (3.0 + this->config.c)*Fssp;
        std::cout << "force embedding: enabled" << std::endl;
        std::cout << "Fssp: " << Fssp << std::endl;
        std::cout << "lb:   " << lbFE << std::endl;
        std::cout << "ub:   " << ubFE << std::endl;
        std::cout << "Fz:   " << this->cache.Fdes(2) << std::endl;
        this->cache.lbA.segment(this->config.qsize+this->config.othercons_size+this->config.holsize,1).setConstant(0.0);
        this->cache.ubA.segment(this->config.qsize+this->config.othercons_size+this->config.holsize,1).setConstant(INFTY);
    }
...
void TSCQPsolver_SLIP_FE::getDebug(VectorXf &dbg) {
