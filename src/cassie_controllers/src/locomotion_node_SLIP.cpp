...
int main(int argc, char *argv[]) {
    ...
    std::ofstream logfileWalk, logfileWalku, logfileStand, logfileInair;
    ...
    if (log_controller) {
        ...
        {
            std::string path = home + "/cassie_ws_2/log/qp_y_stand_log_SLIP.txt";
            logfileStand.open(path);
        }
        {
            std::string path = home + "/cassie_ws_2/log/qp_y_inair_log_SLIP.txt";
            ...
            switch (mode) {
                ...
                case 0 : {
                    if (ros::Time::now().toSec() < 5.0){
                        ...
                        // if (tscqpsolver_inair.shownError){
                            std::cout << "qpCompleted: " << tscqpsolver_inair.qpCompleted << std::endl;
                            return 0;
                        // }
                        ...
                        output_standing.update(radio);
                        tscqpsolver_standing.update(u, output_standing);
                        if (log_controller){
                            output_standing.getDebug(qplogstand);
                            logfileStand << qplogstand << "\n\n";
                        }
                    }
                    ...
                    if (mode_command > 0 && ros::Time::now().toSec() > 6.0) {
                ...
                default :
                    ROS_ERROR("The controller state machine broke itself...");
                    return 1;
            }
        }
