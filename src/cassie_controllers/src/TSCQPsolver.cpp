...
void TSCQPsolver::update(VectorXd &u, OUTPUT_HLIP &output){
    this->output.ya = output.cache.ya;
    this->output.dya = output.cache.dya;
    this->output.Dya = output.cache.Dya;
    this->output.DLfya = output.cache.DLfya;

    this->output.yd = output.cache.yd;
    this->output.dyd = output.cache.dyd;
    this->output.d2yd = output.cache.d2yd;

    this->output.domain = output.getDomain();

    this->cache.u = this->getTorqueQP();
    u = this->cache.u;
}

void TSCQPsolver::update(VectorXd &u, OUTPUT_SLIP &output){
    this->output.ya = output.cache.ya;
    this->output.dya = output.cache.dya;
    this->output.Dya = output.cache.Dya;
    this->output.DLfya = output.cache.DLfya;
...
VectorXd TSCQPsolver::getTorqueQP() {
    // Formulate the QP
    // States are X = [ddq; u; F] \in \mathbb{R}^{43} (22,10,11)

    // Output error
    VectorXd eta(this->config.outputsize*2);
    eta << this->output.ya - this->output.yd,
           this->output.dya - this->output.dyd;

    // std::cout << "eta: " << eta << std::endl;
    // std::cout << "deta: " << this->output.d2yd << std::endl;
    ...
    // 0.5*x'*H*x + f'*x
    // Task space terms
    MatrixXd JY  = this->output.Dya.block(0,0,this->config.outputsize,22) ;
    MatrixXd dJY = this->output.DLfya.block(0,0,this->config.outputsize,22) ;
    // this->cache.b_y = this->output.d2yd - dJY * this->robot->dq - this->config.Kpy.cwiseProduct(eta.segment(0,this->config.outputsize)) - this->config.Kdy.cwiseProduct(eta.segment(this->config.outputsize,this->config.outputsize));
    this->cache.b_y = this->output.d2yd - this->config.Kpy.cwiseProduct(eta.segment(0,this->config.outputsize)) - this->config.Kdy.cwiseProduct(eta.segment(this->config.outputsize,this->config.outputsize));
    ...
    // std::cout << "C: " << this->robot->dynamics.C << std::endl;
    // std::cout << "H: " << this->robot->dynamics.H << std::endl;
    std::cout << "lbA: " << this->cache.lbA << std::endl;
    std::cout << "ubA: " << this->cache.ubA << std::endl;
    std::cout << "G: " << this->cache.G << std::endl;
    std::cout << "g: " << this->cache.g << std::endl;
    std::cout << "Aconstr: " << this->cache.Aconstr << std::endl;

    std::cout << "ya: " << this->output.ya << std::endl;
    std::cout << "dya: " << this->output.dya << std::endl;
    std::cout << "yd: " << this->output.yd << std::endl;
    std::cout << "dyd: " << this->output.dyd << std::endl;
    std::cout << "d2yd: " << this->output.d2yd << std::endl;
    std::cout << "Dya: " << this->output.Dya << std::endl;
    std::cout << "DLfya: " << this->output.DLfya << std::endl;

    // std::cout << "lb: " << this->config.lb << std::endl;
    // std::cout << "ub: " << this->config.ub << std::endl;

    /////////////
    // QPOASES //
    /////////////
    // // Debug printing
    // std::cout << "Gflat: " << this->cache.G << std::endl;
    // std::cout << "g: " << this->cache.g << std::endl;
    // std::cout << "AConstrFlat: " << this->cache.Aconstr << std::endl;
    // std::cout << "lbA: " << this->cache.lbA << std::endl;
    // std::cout << "ubA: " << this->cache.ubA << std::endl;
    // std::cout << "eta: " << eta << std::endl;
    // std::cout << "d2yd: " << this->output.d2yd << std::endl;
...
//        this->memory.u_prev << this->cache.qpsol.segment(this->config.qsize,this->config.usize);
    ...
    std::cout << "usolution: " << this->cache.u << std::endl;
