...
void OUTPUT_HLIP::update(VectorXd &radio) {
    this->memory.time = this->memory.timer.elapsed();
    ...
    // Get the min and max to normalize the xcom position
    double min, max;
    min = -0.14;
    max = 0.16;
...
void OUTPUT_HLIP::update_Ly(double &Ly) {
...
void OUTPUT_HLIP::update_Lx(double &Lx, double &Ly) {
    Vector3d p_com, dp_com, L3d;
    p_com << this->cache.xcLIP, this->cache.ycLIP, this->cache.zcLIP;
    dp_com << this->cache.dxcLIP, this->cache.dycLIP, this->cache.dzcLIP;
    MatrixXd Lcom(6,1);
