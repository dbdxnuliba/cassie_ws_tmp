...
static ros_utilities::Timer total_timer(true);
...
int main(int argc, char *argv[])
{
    ...
    basic_contact_ekf basic_ekf;

    // Whether to log data
    VectorXf log = VectorXf::Zero(5);
    std::fstream logfile;
    ...
    // Whether to log ekf gazebo comparison
    bool log_ekf = true;
    std::ofstream logfileEKF;
    if (log_ekf) {
        std::string home = getenv("HOME");
        std::string path = home + "/cassie_ws_2/log/q_ekf_est.txt";
    ...
    while(ros::ok()){
        ...
        if (isCalibrated) {
            if (useContactEKF){
                ...
                ///////////////
                Matrix3d Rhat;
                basic_ekf.getRotationMatrixXYZ(Rhat,euler.alpha(),euler.beta(),euler.gamma());
                // std::cout << "Rhat: " << Rhat << std::endl;
                basic_ekf.update(true,total_timer.elapsed(),a,encoder,Rhat,con);

                VectorXd p_basic, vel_basic, pLfoot_basic, pRfoot_basic, ba_basic;
                basic_ekf.getValues(p_basic,vel_basic,pLfoot_basic,pRfoot_basic,ba_basic);
                // Rotate into frame relative to stance foot
                if ( (con(0) >= 0.25) && (con(1) >= 0.25) ) {
                    // Double support
                    VectorXd vl(3), vr(3);
                    ...
                    // Right support
                    vel_basic = Rzr * vel_basic;
                } else if (con(0) >= 0.25) {
                    // Left support
                    vel_basic = Rzl * vel_basic;
                }

                out_msg.linear_velocity.x = vel_basic(0);
                out_msg.linear_velocity.y = vel_basic(1);
                out_msg.linear_velocity.z = vel_basic(2);

                if (log_ekf){
                    logfileEKF << con << "\n";
                    logfileEKF << vel << "\n";
                    logfileEKF << vel_basic << "\n";
                }
//                std::cout << "EKF_vel"<< vel << std::endl;
            ...
            // use directly the gazebo forward velocity if we're in simulation
            if (isSim){
                out_msg.linear_velocity.x = Simvel(0);
                out_msg.linear_velocity.y = Simvel(1);
                out_msg.linear_velocity.z = Simvel(2);
                if (log_ekf){
                    logfileEKF << Simvel.segment(0,3) << "\n";
                    logfileEKF << robot.GRF << "\n";
                    logfileEKF << encoder << "\n\n";
                }
            }
...
//            Add header & publish message
           ...
           if (log_estimation){
               ...
               if ( !isSim )
                   tsec -= 1.6074545e9;
               log << static_cast<float>(tsec),static_cast<float>(tnsec),
